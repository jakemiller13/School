...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        text_to_msg = Message(self.message_text)
        correct_words = 0
        correct_shift = 0
        word_list = self.get_valid_words()
        
        for i in range(26):
            next_correct_words = 0
            words = text_to_msg.apply_shift(i).split(' ')
            
#            for j in words: #can you do list comprehension on this?
#                if j in word_list:
#                    next_correct_words += 1
            next_correct_words = len([j for j in words if j in word_list])
        
            if next_correct_words > correct_words:
                correct_words = next_correct_words
                correct_shift = i
                join_words = ' '.join(words)
        
        return (correct_shift,join_words)
...
def decrypt_story():
    '''
    Decrypts given story by applying decrypt_message() function
    '''
